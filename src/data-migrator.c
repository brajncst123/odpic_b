...
main(int argc, char *argv[])
{
    ...
    {
        printf(" Pre-Execute My Sql: %s \n",st_schema_tables[n].query);
        if (dpiConn_prepareStmt(conn, 0, st_schema_tables[n].query, strlen(st_schema_tables[n].query), NULL, 0,
                &stmt) < 0)
            return printError();
        if (dpiConn_newVar(conn, DPI_ORACLE_TYPE_STMT, DPI_NATIVE_TYPE_STMT, 1, 0, 0, 0, NULL, &refCursorVar, 
                &refCursorValue) < 0)
            return printError();
